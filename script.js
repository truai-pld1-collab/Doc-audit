...
// *** ฟังก์ชันแก้ไข: ดึง File ID จาก URL (รองรับ /preview และ /uc) ***
const getFileIdFromUrl = (url) => {
    if (!url) return null;
    let fileId = null;

    // 1. ตรวจสอบรูปแบบ /file/d/[ID]/preview
    let match = url.match(/\/d\/([a-zA-Z0-9_-]+)/);
    if (match && match[1]) {
        fileId = match[1];
    } else {
        // 2. ตรวจสอบรูปแบบ /uc?id=[ID] (รูปแบบเก่าหรือรูปแบบอื่นๆ)
...
const getFullSizeUrl = (url) => {
    ...
    if (!fileId) return '';
    // ใช้ Direct Download link สำหรับ Modal (รูปขนาดเต็ม, เลี่ยงการบล็อก)
