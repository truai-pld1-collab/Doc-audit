...
// *** ฟังก์ชันใหม่: ปรับ URL ให้ดึงภาพขนาดเต็มจาก Drive ***
const getFullSizeUrl = (url) => {
    // ตรวจสอบว่าเป็น URL ของ Drive และมีพารามิเตอร์แล้วหรือไม่
    if (url.includes('drive.google.com/file/d/')) {
        // ลบพารามิเตอร์เก่าออก (ถ้ามี) แล้วต่อท้ายด้วย '=s1600' เพื่อดึงภาพขนาด 1600px
    ...
    fetch(`${scriptURL}?sheet=${encodeURIComponent(sheetName)}`)
      ...
      .then(data => {
        ...
        filteredData.forEach(row => {
          ...
          // *** ใช้ฟังก์ชัน getFullSizeUrl สำหรับ Modal ***
          const modalUrl1 = getFullSizeUrl(image1Url);
          const modalUrl2 = getFullSizeUrl(image2Url);

          // Thumbnail ยังคงใช้ URL เดิม (ขนาดเล็ก)
          const imgTag1 = image1Url ? `<img src="${image1Url}" class="thumbnail" onclick="openModal('${modalUrl1}')">` : ''; 
          const imgTag2 = image2Url ? `<img src="${image2Url}" class="thumbnail" onclick="openModal('${modalUrl2}')">` : ''; 
