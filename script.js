...
const getFileIdFromUrl = (url) => {
    if (!url) return null;
    // รูปแบบ URL ที่ Apps Script บันทึก: https://drive.google.com/file/d/[ID]/preview
...
// *** ฟังก์ชันแก้ไข: ปรับ URL ให้ดึงภาพขนาดใหญ่สำหรับ Modal (Preview Link) ***
const getFullSizeUrl = (url) => {
    const fileId = getFileIdFromUrl(url); 
    if (!fileId) return '';
    // ใช้รูปแบบ /file/d/[ID]/preview ซึ่งเสถียรที่สุดสำหรับ Modal
    ...
    fetch(`${scriptURL}?sheet=${encodeURIComponent(sheetName)}`)
      ...
      .then(data => {
        ...
        filteredData.forEach(row => {
          ...
          // *** THUMBNAIL: ใช้รูปแบบ /thumbnail?id=... (ใช้ File ID) ***
