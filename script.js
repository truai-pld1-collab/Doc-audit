...
// *** ฟังก์ชันแก้ไข: ดึง File ID จาก URL (สำคัญมาก!) ***
...
const getFullSizeUrl = (url) => {
    ...
    if (!fileId) return '';
    // ใช้ Viewer URL (uc?id=...) ซึ่งเสถียรกว่าสำหรับการแสดงผลใน Modal 
    return `https://drive.google.com/uc?id=${fileId}`; 
};
// ********************************************************

// *** ฟังก์ชันแก้ไข: ลบข้อมูลแถวตาม Index (แก้ไขเพื่อให้ทำงานร่วมกับ no-cors ได้) ***
function deleteRow(rowIndex, sheetName) {
    // ต้องตรวจสอบ rowIndex ก่อน
    if (rowIndex === 'N/A' || !rowIndex) {
        alert('ไม่สามารถลบข้อมูลได้: ไม่พบ Row Index ที่ถูกต้อง');
        return;
    }

    if (!confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบข้อมูลแถวที่ ${rowIndex} จากชีต ${sheetName}?`)) {
        return;
    }
    
    // สร้าง URL สำหรับ Apps Script Endpoint เพื่อดำเนินการลบ
    const url = `${scriptURL}?action=delete&row=${rowIndex}&sheet=${encodeURIComponent(sheetName)}`;

    fetch(url, {
        method: 'GET', // ใช้ GET เพื่อส่งคำสั่งลบ
        mode: 'no-cors' // ตั้งค่าโหมด no-cors ตาม Apps Script
    })
    // เมื่อใช้ no-cors จะไม่สามารถตรวจสอบ Response ได้อย่างน่าเชื่อถือ จึงแจ้งเตือน
    // และโหลดข้อมูลใหม่หลังจากหน่วงเวลาเล็กน้อยเพื่อให้ Apps Script มีเวลาทำงาน
    .then(() => {
        setTimeout(() => {
            alert('ส่งคำสั่งลบข้อมูลเรียบร้อยแล้ว (จะโหลดข้อมูลใหม่ในอีกสักครู่)'); 
            debouncedLoadData(); 
        }, 1000); // หน่วงเวลา 1 วินาที
    })
    .catch(error => {
        // ข้อผิดพลาดจากการส่งคำขอ (อาจไม่แสดงผลเมื่อใช้ no-cors)
        console.error('Error sending delete request:', error);
        alert('ข้อผิดพลาดในการส่งคำสั่งลบ โปรดลองอีกครั้ง');
    });
}
// ********************************************************************************
    ...
    fetch(`${scriptURL}?sheet=${encodeURIComponent(sheetName)}`)
      ...
      .then(data => {
        ...
        filteredData.forEach(row => {
          ...
          // *** THUMBNAIL: ใช้รูปแบบ /thumbnail?id=... โดยร้องขอ w200 เพื่อคุณภาพที่ดีขึ้น เมื่อถูกย่อด้วย CSS ***
          const thumbnailSrc1 = fileId1 ? `https://drive.google.com/thumbnail?id=${fileId1}&sz=w200` : ''; 
          const thumbnailSrc2 = fileId2 ? `https://drive.google.com/thumbnail?id=${fileId2}&sz=w200` : '';
          ...
          // *** ดึง RowIndex และเพิ่มปุ่ม Delete ***
...
function loadChartData() {
  ...
  // *** โค้ดที่เพิ่มสำหรับ S1 ***
  fetch(`${scriptURL}?sheet=Audit S1`)
    .then(response => response.json())
    .then(data => {
      const countsS1 = filterAndCountProblems(data, fromDateValue, toDateValue);
      renderChart(countsS1, 'problemChartS1', 'Audit S1');
    });
  // *** โค้ดที่เพิ่มสำหรับ S2 ***
  fetch(`${scriptURL}?sheet=Audit S2`)
    .then(response => response.json())
    .then(data => {
      const countsS2 = filterAndCountProblems(data, fromDateValue, toDateValue);
      renderChart(countsS2, 'problemChartS2', 'Audit S2');
    });
  // *** โค้ดที่เพิ่มสำหรับ SP ***
  fetch(`${scriptURL}?sheet=Audit SP`)
    .then(response => response.json())
    .then(data => {
      const countsSP = filterAndCountProblems(data, fromDateValue, toDateValue);
      renderChart(countsSP, 'problemChartSP', 'Audit SP');
    });
}
...
function openModal(src) {
  ...
  const fullSizeLink = document.getElementById("fullSizeLink"); // ดึงปุ่ม/ลิงก์
  
  modal.style.display = "flex"; 
  modalImg.src = src;
  
  // *** กำหนดลิงก์ขนาดเต็มให้กับปุ่ม/ลิงก์ใน Modal ***
